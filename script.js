// ==============================
// Frase de intro aleatoria en cada visita
// ==============================
const introLine = document.getElementById('intro-line');
const introPhrases = [
  'Para la persona que hace florecer mi corazón',
  'Para la razón de mis mejores días',
  'Para mi tulipán favorito entre todas las flores',
  'Para la persona que le da color a mi mundo',
  'Para ti, que haces que todo florezca'
];

if (introLine){
  introLine.textContent = introPhrases[Math.floor(Math.random() * introPhrases.length)];
}
...
// Días juntos
// ==============================
// ✏️ Cambia esta fecha por el día que empezaron su relación (año, mes-1, día)
const RELATIONSHIP_START_DATE = new Date(2025, 10, 20); // 20 de noviembre de 2025
...
function updateDaysTogether(){
  ...
  el.textContent = `Llevamos ${days} día${days === 1 ? '' : 's'} juntos 💛🌷`;
}
...
// ==============================
// Carta escondida (se desbloquea el 21 de septiembre a medianoche)
// ==============================
const openLetterBtn = document.getElementById('open-letter-btn');
const letterBtnText = document.getElementById('letter-btn-text');
const letterCaption = document.getElementById('letter-caption');
...
function updateLetterLock(){
  const unlockedNow = isSept21Today();
  if (unlockedNow === letterUnlocked) return; // sin cambios, no repetir trabajo

  letterUnlocked = unlockedNow;

  if (letterUnlocked){
    openLetterBtn.disabled = false;
    openLetterBtn.classList.remove('locked');
    openLetterBtn.classList.add('unlocked');
    letterBtnText.textContent = '💌 Abrir carta';
    letterCaption.textContent = '¡Ya se puede abrir, Paullette! 💛';
  } else {
    openLetterBtn.disabled = true;
    openLetterBtn.classList.add('locked');
    openLetterBtn.classList.remove('unlocked');
    letterBtnText.textContent = '🔒 Todavía no';
    letterCaption.textContent = 'Se desbloquea el 21 de septiembre a medianoche 🔒';
  }
}
...
updateLetterLock();
setInterval(updateLetterLock, 1000);
...
const CAROUSEL_INTERVAL_MS = 4000; // tiempo entre cada cambio de foto (ajusta aquí)
...
function startCarouselAutoplay(){
  ...
  carouselTimer = setInterval(goToNextSlide, CAROUSEL_INTERVAL_MS);
}
...
prevBtn.addEventListener('click', () => {
  ...
  updateCarousel();
  startCarouselAutoplay(); // reinicia el conteo tras una interacción manual
});
...
// Pausa el auto-avance mientras el mouse está sobre la galería (opcional)
...
// ==============================
// Música de fondo con playlist
// ==============================
// ✏️ Sube tus mp3 a esta misma carpeta y agrégalos aquí (nombre de archivo + título a mostrar).
// Si dejas el arreglo vacío o los archivos no existen, el botón simplemente no reproducirá nada.
const MUSIC_TRACKS = [
  { src: 'WhatsApp Audio 2026-09-16 at 7.50.03 PM.mp4', title: 'Nuestra canción' },
];
...
// Al terminar una canción, pasa automáticamente a la siguiente de la lista
